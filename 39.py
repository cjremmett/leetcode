...
class Solution:
    def combinationSum(self, candidates: List[int], target: int) -> List[List[int]]:
        ...
        for i in range(0, len(candidates)):
            self.iterate_num(c, cur_cand, candidates)

        

    def iterate_num(self, progress, to_add, sum, candidates):
        if len(self.return_list) >= 150:
            return
        ...
        if sum > target:
            ...
            for i in range(0, len(candidates)):
                self.iterate_num(progress, candidate, sum, candidates[i:])
